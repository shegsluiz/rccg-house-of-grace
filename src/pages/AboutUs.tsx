...
export default function AboutUs() {
  return (
    <div className="bg-white text-gray-900 min-h-screen font-sans selection:bg-green-500/30 overflow-x-hidden">
      ...
      <section className="relative min-h-[100dvh] flex flex-col justify-end px-6 md:px-12 lg:px-8 pb-16 md:pb-24 overflow-hidden">
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full">
          <p className="text-[11px] font-bold tracking-[0.22em] uppercase text-green-700 mb-5 flex items-center gap-3">
          ...
          <p className="text-gray-600 text-lg md:text-xl max-w-lg leading-relaxed mt-6 mb-10 font-medium">
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        ...
        <FadeSection delay={120}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <blockquote className="text-2xl md:text-3xl font-bold italic leading-relaxed text-gray-800 border-l-4 border-green-500 pl-8 py-2">
      ...
      {/* ── Core Values ──────────────────────────────────────────────────────── */}
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 bg-gray-50 w-full rounded-[2.5rem] my-12">
        <FadeSection>
          <p className="text-green-600 text-sm font-bold uppercase tracking-widest mb-4">Our Core Values</p>
          <h2 className="text-4xl sm:text-5xl font-bold leading-tight mb-16">What Defines Us</h2>
        </FadeSection>
        <FadeSection delay={100}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {values.map((v) => (
              <div key={v.number} className="bg-white border border-gray-200 shadow-sm p-10 rounded-3xl hover:shadow-md hover:border-green-200 transition-all group">
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        ...
        <FadeSection delay={120}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {leadership.map((l) => (
              <div key={l.role} className="bg-white border border-gray-200 shadow-sm p-8 rounded-3xl relative overflow-hidden group hover:border-amber-400 transition-all">
                ...
                <div className="flex flex-col items-center text-center gap-6">
                  {l.image && (
                    <img 
                      src={l.image} 
                      alt={l.name} 
                      className="w-40 h-40 md:w-48 md:h-48 rounded-full object-cover border-2 border-gray-100 shadow-md shrink-0" 
                    />
                  )}
                  <div>
                    <p className="text-[10px] tracking-widest uppercase text-amber-600 font-bold mb-3">{l.role}</p>
                    <p className="text-2xl font-bold text-gray-900 mb-2">{l.name}</p>
                    <p className="text-sm text-gray-600 tracking-wide">{l.note}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
            {whatToExpect.map((item) => (
              <div key={item.title} className="flex gap-6 items-start group">
                <div className="w-14 h-14 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center text-xl text-green-600 group-hover:scale-110 transition-transform shrink-0 shadow-sm">
                  {item.icon}
                </div>
                <div>
      ...
      <FadeSection>
        <div className="my-16 md:my-24 px-6 md:px-12 lg:px-8 w-full max-w-7xl mx-auto">
          <div className="bg-green-50 border border-green-100 rounded-3xl p-12 lg:p-20 flex flex-col lg:flex-row items-center justify-between gap-10 shadow-sm max-w-7xl mx-auto">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight text-center lg:text-left text-gray-900">
