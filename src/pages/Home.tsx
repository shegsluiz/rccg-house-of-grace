...
export default function Home() {
  ...
  return (
    <div className="bg-white text-gray-900 min-h-screen font-sans selection:bg-green-500/30">
      ...
      <section className="relative min-h-[100dvh] flex flex-col justify-end px-6 md:px-12 lg:px-8 pb-16 md:pb-24 overflow-hidden">
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full">
          <p className="text-[11px] font-bold tracking-[0.22em] uppercase text-green-600 mb-5 flex items-center gap-3">
            <span className="w-8 h-[1px] bg-green-600"></span> Welcome to
          </p>
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold leading-[1.05] tracking-tight text-gray-900 max-w-4xl">
            RCCG<br />
            House of <em className="not-italic bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text text-transparent">Grace</em>
          </h1>
          <p className="text-gray-700 text-lg md:text-xl max-w-lg leading-relaxed mt-6 mb-10 font-medium">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Link to="/about" className="bg-green-50 border border-green-200 px-8 py-4 rounded-full font-bold hover:bg-green-100 transition-all duration-300 text-green-700 text-sm tracking-wide uppercase">
              Who We Are
            </Link>
          </div>
        </div>
        ...
        <div className="absolute bottom-10 right-6 md:right-12 lg:right-24 z-20 flex flex-col items-center gap-2 opacity-60">
          ...
          <span className="text-[10px] tracking-[0.2em] uppercase text-green-600" style={{ writingMode: 'vertical-rl' }}>Scroll</span>
        </div>
      </section>

      {/* ── Welcome / Pastor Message ──────────────────────────────────────────── */}
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        <FadeSection>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="relative aspect-[4/5] max-w-md w-full mx-auto lg:mx-0 rounded-3xl overflow-hidden shadow-sm border border-gray-100">
              <img src="/IMG_6384.jpg" alt="Welcome to RCCG House of Grace" className="absolute inset-0 w-full h-full object-cover" />
            </div>
            <div>
              <p className="text-green-600 text-sm font-bold uppercase tracking-widest mb-4">A Word From Our Pastor</p>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        ...
        <FadeSection delay={120}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pillars.map((p) => (
              <div key={p.word} className="bg-gray-50 border border-gray-100 p-10 rounded-3xl hover:bg-gray-100 transition-all group">
                <p className="text-6xl font-bold text-gray-200 group-hover:text-green-500/20 transition-colors mb-4">{p.letter}</p>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        ...
        <FadeSection delay={100}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceTimes.map((s) => (
              <div key={s.name} className="bg-gray-50 border border-gray-100 p-10 rounded-3xl hover:bg-gray-100 transition-all">
                ...
                <p className="text-lg font-bold text-gray-900 mb-4">{s.name}</p>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        <FadeSection>
          <p className="text-green-600 text-sm font-bold uppercase tracking-widest mb-4">Hear The Word</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className="relative aspect-video rounded-3xl overflow-hidden border border-gray-100 bg-gray-50">
              {sermon.youtubeId === "YOUR_YOUTUBE_VIDEO_ID" ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-green-600/50">
            ...
            <div>
              <p className="text-green-600 text-xs font-bold tracking-widest uppercase mb-4">{sermon.date}</p>
              <h3 className="text-3xl md:text-4xl font-bold mb-4">{sermon.title}</h3>
              <p className="text-gray-600 font-medium mb-2">{sermon.preacher}</p>
              <p className="text-emerald-600 italic mb-8">{sermon.scripture}</p>
              <Link to="/sermons" className="bg-green-50 border border-green-200 px-8 py-4 rounded-full font-bold hover:bg-green-100 transition-all duration-300 text-green-700 text-sm tracking-wide uppercase">
                View All Sermons
              </Link>
            </div>
          </div>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-gray-200 w-full">
        <FadeSection>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <Link to="/events" className="text-gray-900 hover:text-orange-600 transition-colors font-bold flex items-center gap-2">
        ...
        <FadeSection delay={120}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((ev, i) => (
              <div key={i} className="bg-gray-50 border border-gray-100 p-10 rounded-3xl hover:border-orange-300 transition-all relative overflow-hidden group">
                ...
                <p className="text-gray-600 leading-relaxed text-sm">{ev.description}</p>
              </div>
            ))}
          </div>
        </FadeSection>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto w-full">
        <FadeSection>
          <div className="bg-gradient-to-br from-green-50 to-white border border-green-200 rounded-3xl p-10 md:p-16 text-center max-w-4xl mx-auto shadow-sm">
            ...
            {!subscribed ? (
              <form className="flex flex-col sm:flex-row max-w-md mx-auto gap-4" onSubmit={handleSubscribe}>
                <input
                  ...
                  required
                  className="flex-1 bg-white border border-gray-200 rounded-full px-6 py-4 focus:outline-none focus:border-green-400 text-gray-900 placeholder:text-gray-400"
                />
              ...
              <p className="text-green-600 font-bold">✦ Thank you — you're on the list. God bless you.</p>
            )}
      ...
      {/* ── RCCG Affiliation ──────────────────────────────────────────────────── */}
      <div className="border-t border-gray-200 py-8 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto w-full flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <span className="px-3 py-1 border border-gray-200 rounded-full text-[10px] tracking-widest uppercase text-gray-500">A Parish of RCCG</span>
          <span className="text-xs text-gray-500 font-medium">Zonal HQ · Lagos Province 15</span>
        </div>
        <div className="flex gap-6">
          <a href="https://www.rccg.org" target="_blank" rel="noreferrer" className="text-xs font-bold tracking-widest uppercase text-gray-500 hover:text-gray-900 transition-colors">RCCG HQ ↗</a>
        </div>
      </div>
    </div>
  );
