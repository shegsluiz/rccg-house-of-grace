...
export default function KingdomCinema() {
  return (
    <div className="bg-black text-white min-h-screen font-sans overflow-x-hidden selection:bg-green-900/40">
      ...
      <section className="relative min-h-[100dvh] flex flex-col justify-end px-6 md:px-12 lg:px-8 pb-16 md:pb-24 pt-40 overflow-hidden">
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full text-center mt-32">
          ...
          <p className="text-zinc-800 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed italic font-medium">
            "Go into all the world and preach the gospel to all creation."<br />
            <span className="text-green-300 not-italic text-sm font-bold uppercase tracking-widest mt-4 inline-block">— Mark 16:15</span>
          </p>
        </div>
      </section>

      {/* ── Vision ── */}
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-5xl mx-auto text-center border-b border-zinc-800 w-full">
        <FadeSection>
      ...
      {/* ── Mission ── */}
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-zinc-800 w-full">
            ...
            <div className="space-y-8">
              {missionPoints.map((item, i) => (
                <div key={i} className="flex gap-6 items-start group">
                  <span className="text-2xl md:text-3xl font-bold text-gray-300 group-hover:text-green-300 transition-colors">{item.num}</span>
                  <p className="text-lg md:text-xl text-zinc-800 leading-relaxed">{item.text}</p>
                </div>
              ))}
            </div>
          </FadeSection>
          <FadeSection delay={150} className="order-1 lg:order-2">
            <div className="relative aspect-[4/5] rounded-3xl overflow-hidden border border-zinc-700 bg-zinc-900 shadow-sm flex items-center justify-center p-12">
              <img 
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto border-b border-zinc-800 w-full">
        <FadeSection text-center>
          <p className="text-green-300 text-sm font-bold uppercase tracking-widest mb-4 text-center">Kingdom Cinema Creates</p>
        ...
        <FadeSection delay={100}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {whatWeDo.map((item, i) => (
              <div key={i} className="bg-black border border-zinc-700 p-8 rounded-3xl hover:bg-zinc-900 hover:shadow-md hover:border-green-500/30 transition-all flex items-center gap-6 group">
                <div className="text-3xl font-bold text-green-200 group-hover:text-green-400 transition-colors">
                  0{i + 1}.
                </div>
                <h3 className="text-xl md:text-2xl font-semibold text-white group-hover:text-green-300 transition">{item}</h3>
              </div>
            ))}
          </div>
        </FadeSection>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <FadeSection>
            <div className="relative aspect-video rounded-3xl overflow-hidden border border-zinc-700 bg-zinc-900 shadow-sm">
              <img 
          ...
          <FadeSection delay={150}>
            <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold leading-tight mb-8 text-white">
            ...
            <div className="space-y-10">
              ...
              <div>
                ...
                <p className="text-lg text-zinc-800 leading-relaxed max-w-sm">
